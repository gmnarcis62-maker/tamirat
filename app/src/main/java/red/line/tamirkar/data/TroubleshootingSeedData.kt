...
object TroubleshootingSeedData {
    ...
    fun defaultGuides(): List<TroubleshootingGuide> = listOf(
        ...
        // =========================================================================
        // دسته‌ی جدید و اختصاصی: روشن نشدن / خاموشی کامل دستگاه — به تفکیک برند و مدل
        // این دسته دقیقاً برای سناریویی طراحی شده که مشتری با شکایت «گوشی روشن نمی‌شود»
        // مراجعه می‌کند؛ هر مورد علت رایج مخصوص همان برند/مدل را دارد.
        // =========================================================================

        // ---------- عمومی (همه برندها) ----------
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "همه برندها", model = "همه مدل‌ها",
            title = "دستگاه کاملاً مرده است — هیچ لرزش، صدا یا نوری ندارد",
            symptoms = "با فشار دکمه پاور یا اتصال شارژر، مطلقاً هیچ واکنشی (نه لرزش، نه لوگو، نه گرم شدن) دیده نمی‌شود",
            possibleCauses = "باتری کاملاً خالی/خراب یا قطع از برد، خرابی IC تغذیه اصلی (PMIC)، اتصالی کوتاه روی برد بعد از ضربه یا نفوذ رطوبت، خرابی پورت شارژ",
            solutionSteps = "۱. تست با باتری و شارژر سالم دیگر به‌صورت جداگانه\n۲. اندازه‌گیری جریان مصرفی دستگاه با منبع تغذیه آزمایشگاهی (Power Supply) به‌جای باتری\n۳. اگر جریان بی‌نهایت بالا کشید (اتصال کوتاه)، با ترموکمرا یا انگشت (با احتیاط) نقطه‌ی داغ روی برد را پیدا کنید\n۴. بررسی مقاومت خطوط تغذیه اصلی با مولتی‌متر در حالت دیود",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "همه برندها", model = "همه مدل‌ها",
            title = "با اتصال به شارژر فقط یک لرزش کوتاه می‌زند و خاموش می‌ماند",
            symptoms = "اتصال شارژر باعث یک ویبره کوتاه می‌شود ولی صفحه روشن نمی‌ماند و لوگو نمی‌آید",
            possibleCauses = "بوت‌لودر خراب، خرابی جزئی حافظه eMMC/UFS، ولتاژ ناکافی از IC تغذیه برای روشن نگه‌داشتن پردازنده",
            solutionSteps = "۱. تست ورود به Recovery/Download/DFU Mode (بسته به برند) برای بررسی زنده بودن پردازنده\n۲. در صورت ورود موفق به این حالت‌ها، مشکل نرم‌افزاری است — فلش مجدد فرم‌ور\n۳. در صورت عدم ورود، بررسی سخت‌افزاری برد (IC تغذیه و حافظه) با باکس تخصصی",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "همه برندها", model = "همه مدل‌ها",
            title = "دستگاه بعد از افتادن روی زمین خاموش شد و دیگر روشن نمی‌شود",
            symptoms = "درست قبل از ضربه دستگاه سالم بوده و بلافاصله بعد از افتادن خاموش شده است",
            possibleCauses = "جدا شدن کانکتور باتری از برد، شکستگی لایه‌ی برد (Layer Crack) در اثر ضربه، جابجایی جزئی IC",
            solutionSteps = "۱. باز کردن دستگاه و بررسی محکم بودن کانکتور باتری\n۲. بازرسی چشمی برد زیر لوپ برای ترک خوردگی احتمالی\n۳. تست ولتاژ نقاط کلیدی برد طبق شماتیک مدل (در صورت دسترسی به شماتیک)",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "همه برندها", model = "همه مدل‌ها",
            title = "دستگاه گاهی روشن می‌شود و گاهی نه (روشن‌شدن نامنظم)",
            symptoms = "با چند بار تلاش، بعضی اوقات روشن می‌شود و بعضی اوقات کاملاً بی‌واکنش است",
            possibleCauses = "باتری وارفته یا متورم با اتصال ضعیف، لحیم سرد (Cold Joint) روی یکی از IC های برد، اتصال ضعیف کانکتور باتری",
            solutionSteps = "۱. تست با باتری کاملاً سالم دیگر\n۲. فشار ملایم روی نقاط مختلف برد حین روشن کردن برای یافتن نقطه‌ی اتصال ضعیف (تکنیک Flex Test)\n۳. ری‌فلو یا ری‌بال نقطه‌ی مشکوک با دستگاه هیت‌گان توسط بردکار",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "همه برندها", model = "همه مدل‌ها",
            title = "بعد از تعمیر یا تعویض قطعه توسط تعمیرکار قبلی، دستگاه اصلاً روشن نمی‌شود",
            symptoms = "قبل از تعمیر قبلی سالم بوده، بلافاصله بعد از آن تعمیر خاموش مانده",
            possibleCauses = "اتصالی هنگام لحیم‌کاری، فلت اشتباه یا کج وصل‌شده، پیچ‌های بیش‌ازحد سفت‌شده که به برد فشار آورده",
            solutionSteps = "۱. باز کردن مجدد و بررسی دقیق تمام فلت‌ها و کانکتورهای مرتبط با تعمیر قبلی\n۲. بررسی نشانه‌های سوختگی یا لحیم اضافی\n۳. تست جداگانه‌ی قطعه‌ی تعویض‌شده با یک نمونه‌ی سالم دیگر",
            difficulty = "متوسط"
        ),
        ...
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Apple", model = "iPhone 6 / 6s / 6s Plus",
            title = "روشن نمی‌شود و حتی با شارژر هم هیچ واکنشی ندارد (مشکل معروف No Power)",
            symptoms = "دستگاه کاملاً مرده است؛ نه با دکمه پاور و نه با اتصال به شارژر/کامپیوتر هیچ واکنشی نشان نمی‌دهد",
            possibleCauses = "این یکی از شناخته‌شده‌ترین ایرادهای این دو مدل است: خرابی آی‌سی معروف به Tristar (U2) که مسئول مدیریت برق USB و شارژ است و معمولاً بر اثر اتصال کابل غیراستاندارد یا ضربه آسیب می‌بیند",
            solutionSteps = "۱. اندازه‌گیری مقاومت خط تغذیه مرتبط با Tristar با مولتی‌متر (در حالت اتصال کوتاه، مقاومت نزدیک صفر است)\n۲. در صورت تایید اتصالی، این تعمیر نیازمند حذف/جامپر یا تعویض U2 (Tristar) با میکروسولدرینگ تخصصی است\n۳. این تعمیر را فقط بردکار مجرب با تجربه‌ی آیفون انجام دهد؛ ریسک آسیب بیشتر به برد در صورت عدم مهارت وجود دارد",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Apple", model = "iPhone 7 / 7 Plus",
            title = "روشن نمی‌شود و توسط آیتونز/کامپیوتر هم شناسایی نمی‌شود (مشکل معروف Audio IC)",
            symptoms = "دستگاه کاملاً بی‌پاسخ است و در Device Manager ویندوز یا آیتونز هم چیزی شناسایی نمی‌شود",
            possibleCauses = "ایراد شناخته‌شده‌ی آیفون ۷: خرابی آی‌سی صدا (Audio IC) که به دلیل نزدیکی به مدار تغذیه، باعث قطع کامل برق دستگاه می‌شود",
            solutionSteps = "۱. تست جریان مصرفی با منبع تغذیه آزمایشگاهی برای تایید اتصالی\n۲. در صورت تایید، بای‌پس یا تعویض Audio IC توسط بردکار متخصص (کار تخصصی میکروسولدرینگ)\n۳. توضیح به مشتری که این تعمیر معمولاً گران‌تر از تعمیرهای معمول است",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Apple", model = "iPhone X",
            title = "صفحه به‌طور ناگهانی سبز رنگ شده، فریز می‌کند و سپس خاموش/ریست می‌شود",
            symptoms = "قبل از خاموشی کامل، الگوی معروف «صفحه سبز» (Green Screen of Death) دیده می‌شود",
            possibleCauses = "ایراد شناخته‌شده‌ی iPhone X: اتصال ضعیف یا آسیب IC درایور نمایشگر که باعث قطع ارتباط و ریست ناگهانی سیستم می‌شود",
            solutionSteps = "۱. در برخی موارد، ری‌بالینگ IC نمایشگر روی برد مشکل را موقتاً رفع می‌کند\n۲. راه‌حل قطعی و پایدارتر معمولاً تعویض کامل نمایشگر (با قطعه باکیفیت) است\n۳. به مشتری اطلاع دهید این یک ایراد شناخته‌شده‌ی این مدل خاص است، نه لزوماً ناشی از بی‌احتیاطی",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Apple", model = "iPhone 8 تا 15",
            title = "بعد از تعویض باتری (غیر از سرویس اپل) دستگاه خاموش شد و روشن نمی‌شود",
            symptoms = "بلافاصله بعد از تعویض باتری در یک تعمیرگاه دیگر، مشکل شروع شده",
            possibleCauses = "باتری با چیپ تشخیص هویت ناسازگار، اتصال نادرست کانکتور باتری، آسیب فیزیکی حین تعویض",
            solutionSteps = "۱. بررسی محکم و صحیح بودن اتصال کانکتور باتری به برد\n۲. تست با یک باتری اورجینال یا باکیفیت دیگر\n۳. بررسی پیام‌های هشدار سلامت باتری بعد از روشن شدن مجدد",
            difficulty = "متوسط"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Apple", model = "همه مدل‌های آیفون",
            title = "دستگاه با اتصال به کامپیوتر در حالت DFU/Recovery شناسایی می‌شود ولی خودش روشن نمی‌شود",
            symptoms = "صفحه سیاه است ولی نرم‌افزار Finder/iTunes دستگاه را در حالت بازیابی می‌شناسد",
            possibleCauses = "این نشانه‌ی خوبی است: پردازنده و تغذیه‌ی اصلی سالم است و مشکل نرم‌افزاری (فرم‌ور خراب) است، نه سخت‌افزاری",
            solutionSteps = "۱. تلاش برای Restore از طریق Finder/iTunes (توجه: اطلاعات پاک می‌شود مگر بک‌آپ موجود باشد)\n۲. اگر Restore هم با خطا مواجه شد (مثل خطای 4013/9)، احتمال خرابی جزئی حافظه NAND مطرح می‌شود\n۳. در صورت تکرار خطا، بررسی تخصصی با ابزار NAND",
            difficulty = "متوسط"
        ),
        ...
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Samsung", model = "سری Galaxy A (A10 تا A54)",
            title = "دستگاه کاملاً مرده است — رایج‌ترین ایراد این سری",
            symptoms = "نه لرزش، نه لوگو، نه واکنش به شارژر",
            possibleCauses = "شایع‌ترین علت در سری Galaxy A، آسیب به IC تغذیه اصلی (PMIC) در اثر استفاده از شارژر تقلبی/بی‌کیفیت یا نفوذ جزئی رطوبت از منافذ بدنه",
            solutionSteps = "۱. اندازه‌گیری ولتاژ روی پد باتری با مولتی‌متر (باید ولتاژ مشخصی از PMIC خارج شود)\n۲. در صورت عدم وجود ولتاژ خروجی، بررسی و ری‌بال/تعویض PMIC توسط بردکار\n۳. بررسی زیر شیلدهای برد برای آثار خوردگی رطوبت پیش از هر اقدام دیگر",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Samsung", model = "سری Galaxy S8 تا Note10",
            title = "بعد از خالی ماندن طولانی‌مدت باتری، دستگاه دیگر روشن نمی‌شود",
            symptoms = "کاربر می‌گوید دستگاه مدت زیادی (چند هفته یا بیشتر) کنار گذاشته شده و شارژ نکرده بود",
            possibleCauses = "افت عمیق ولتاژ باتری (Deep Discharge) باعث می‌شود IC محافظ داخلی باتری برای ایمنی مدار را قطع کند",
            solutionSteps = "۱. شارژ باتری به‌صورت جداگانه (خارج از گوشی) با شارژر آزمایشگاهی و جریان محدود (Trickle Charge) به مدت چند دقیقه تا چند ساعت\n۲. بعد از رسیدن ولتاژ به حد قابل قبول، اتصال مجدد به برد و تست روشن شدن\n۳. اگر باتری اصلاً ولتاژ نگرفت، تعویض باتری ضروری است",
            difficulty = "متوسط"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Samsung", model = "سری Galaxy S20 به بعد",
            title = "دستگاه فقط در حالت Download Mode (صفحه زرد) بالا می‌آید و روشن عادی نمی‌شود",
            symptoms = "با روشن کردن، فقط صفحه‌ی هشدار زرد رنگ Odin/Download Mode دیده می‌شود",
            possibleCauses = "فلش ناقص یا خراب قبلی، خرابی پارتیشن بوت سیستم",
            solutionSteps = "۱. اتصال به کامپیوتر و فلش مجدد فرم‌ور رسمی و دقیقاً مطابق مدل/منطقه با Odin\n۲. در صورت تکرار مشکل بعد از فلش صحیح، احتمال خرابی سخت‌افزاری حافظه مطرح می‌شود",
            difficulty = "تخصصی"
        ),
        ...
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Xiaomi", model = "سری Redmi Note (8 تا 13)",
            title = "دستگاه روشن نمی‌شود و به‌شدت داغ می‌کند",
            symptoms = "با فشار دکمه پاور یا وصل شارژر، بخشی از برد (نزدیک دوربین یا لبه) خیلی سریع داغ می‌شود",
            possibleCauses = "اتصال کوتاه روی مدار تغذیه، معمولاً بعد از استفاده طولانی‌مدت از شارژر غیراستاندارد یا آب‌خوردگی جزئی گذشته",
            solutionSteps = "۱. فوراً از شارژ کردن مجدد خودداری کنید (خطر آسیب بیشتر)\n۲. پیدا کردن نقطه‌ی داغ با ترموکمرا یا لمس محتاطانه برای یافتن IC معیوب\n۳. رفع اتصالی و تعویض IC آسیب‌دیده توسط بردکار",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Xiaomi", model = "همه مدل‌های شیائومی/ردمی",
            title = "دستگاه در حالت EDL Mode گیر کرده (صفحه کاملاً مشکی، فقط با کامپیوتر شناسایی می‌شود)",
            symptoms = "دستگاه روی کامپیوتر به‌عنوان «Qualcomm HS-USB QDLoader 9008» شناسایی می‌شود ولی صفحه‌ی خودش روشن نمی‌شود",
            possibleCauses = "خرابی شدید نرم‌افزاری یا فلش ناقص که دستگاه را به حالت اضطراری EDL انداخته",
            solutionSteps = "۱. فلش کامل فرم‌ور رسمی در حالت EDL با نرم‌افزار Mi Flash Tool\n۲. استفاده از فایل فرم‌ور Fastboot دقیقاً مطابق کد مدل دستگاه",
            difficulty = "تخصصی"
        ),
        ...
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "Huawei/Honor", model = "همه مدل‌ها",
            title = "بعد از بروزرسانی EMUI/HarmonyOS، دستگاه دیگر روشن نمی‌شود",
            symptoms = "روند آپدیت قطع شده یا با خطا متوقف شده و از آن پس دستگاه بوت نمی‌شود",
            possibleCauses = "خرابی پارتیشن سیستم حین نصب ناقص بروزرسانی",
            solutionSteps = "۱. تلاش برای ورود به eRecovery و نصب مجدد سیستم از طریق اینترنت\n۲. در صورت عدم دسترسی به eRecovery، استفاده از ابزار رسمی هوآوی (مثل DC Phoenix) توسط مرکز مجاز یا بردکار مجهز",
            difficulty = "تخصصی"
        ),
        ...
        TroubleshootingGuide(
            category = TroubleshootingCategory.POWER_ON_FAILURE,
            brand = "سایر برندها (Nokia، Realme، Oppo، Vivo و...)",
            model = "همه مدل‌ها",
            title = "روشن نشدن دستگاه — رویکرد عمومی عیب‌یابی وقتی شماتیک برند در دسترس نیست",
            symptoms = "دستگاه روشن نمی‌شود و مستندات فنی اختصاصی این برند در دسترس تعمیرگاه نیست",
            possibleCauses = "علل مشابه سایر برندها: باتری، IC تغذیه، اتصالی برد، پورت شارژ",
            solutionSteps = "۱. شروع با بررسی‌های عمومی: باتری، پورت شارژ، فلت‌های اصلی\n۲. اندازه‌گیری جریان مصرفی با منبع تغذیه آزمایشگاهی برای تشخیص اتصالی\n۳. جستجوی مدل دقیق در منابع تخصصی تعمیرات (فروم‌ها و کانال‌های تخصصی بردکاری) برای ایرادهای شناخته‌شده‌ی همان مدل\n۴. در صورت عدم تخصص کافی روی این برند خاص، ارجاع به مرکز تعمیر تخصصی‌تر را به مشتری پیشنهاد دهید",
            difficulty = "تخصصی"
        ),

        // =========================================================================
        // موارد تکمیلی مخصوص مدل در سایر دسته‌ها (نه فقط روشن‌نشدن)
        // =========================================================================
        TroubleshootingGuide(
            category = TroubleshootingCategory.DISPLAY,
            brand = "Apple", model = "iPhone 11 / 12 / 13",
            title = "بعد از تعویض نمایشگر غیراصلی، پیام «نمایشگر غیر اورجینال» و افت روشنایی خودکار",
            symptoms = "هشدار زرد رنگ درباره صحت نمایشگر و عدم تنظیم صحیح روشنایی/True Tone",
            possibleCauses = "نمایشگر تعویضی فاقد چیپ همتاسازی (Pairing) با برد اصلی است",
            solutionSteps = "۱. استفاده از نمایشگر با کیفیت «اورجینال-تعمیراتی» (Service Pack) در صورت امکان\n۲. برای رفع کامل هشدار، نیاز به دستگاه پروگرامر تخصصی جهت انتقال سریال بین نمایشگر قدیم و جدید است\n۳. توضیح شفاف به مشتری درباره تفاوت کیفیت و قیمت گزینه‌های مختلف",
            difficulty = "تخصصی"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.CAMERA,
            brand = "Samsung", model = "سری Galaxy A (A30 تا A54)",
            title = "دوربین اصلی مات یا تار عکس می‌گیرد ولی دوربین سلفی سالم است",
            symptoms = "فقط کیفیت دوربین پشت افت کرده، دوربین جلو مشکلی ندارد",
            possibleCauses = "در این سری معمولاً رطوبت پشت لنز دوربین اصلی یا خرابی موتور فوکوس خودکار علت شایع است",
            solutionSteps = "۱. بررسی چشمی لنز برای بخار یا لکه پشت شیشه\n۲. تست فوکوس دستی از حالت Pro دوربین\n۳. تعویض ماژول دوربین اصلی در صورت تایید خرابی فیزیکی",
            difficulty = "متوسط"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.BATTERY_CHARGING,
            brand = "Xiaomi", model = "سری Redmi Note (10 به بعد)",
            title = "شارژ سریع (Fast Charging) کار نمی‌کند و سرعت شارژ به‌شدت پایین است",
            symptoms = "با شارژر اورجینال هم، سرعت شارژ در حد شارژرهای معمولی است",
            possibleCauses = "کابل غیراورجینال بدون پشتیبانی از پروتکل شارژ سریع شیائومی، یا خرابی IC کنترل شارژ سریع",
            solutionSteps = "۱. بررسی فعال بودن گزینه شارژ سریع در تنظیمات MIUI\n۲. تست با کابل و آداپتور اورجینال دیگر (کابل باید از نوع پشتیبانی‌شده باشد)\n۳. بررسی IC شارژ سریع روی برد در صورت تکرار با لوازم اورجینال",
            difficulty = "متوسط"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.BIOMETRIC,
            brand = "Samsung", model = "سری Galaxy S (S21 به بعد) و Note",
            title = "اثر انگشت زیر صفحه با تاخیر زیاد یا به‌ندرت کار می‌کند",
            symptoms = "برخلاف قبل، سنسور اثر انگشت اپتیکال زیر نمایشگر واکنش کند یا نامنظم دارد",
            possibleCauses = "گلس محافظ غیراستاندارد که نور سنسور نوری را مسدود می‌کند، کثیفی زیر گلس، بروزرسانی نرم‌افزاری اخیر",
            solutionSteps = "۱. حذف گلس محافظ فعلی و تست مجدد\n۲. تمیزکاری سطح نمایشگر و انگشت\n۳. حذف و ثبت مجدد اثر انگشت با فشار یکنواخت\n۴. در صورت ادامه مشکل بعد از این مراحل، بررسی سلامت خود سنسور",
            difficulty = "آسان"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.CHARGING_PORT,
            brand = "Apple", model = "iPhone 15 و بالاتر (پورت USB-C)",
            title = "پورت USB-C شارژ نمی‌شود یا انتقال داده ندارد",
            symptoms = "با کابل USB-C، نه شارژ می‌شود و نه به کامپیوتر متصل می‌شود",
            possibleCauses = "ورود گردوغبار/رطوبت به پورت نسبتاً جدید USB-C، آسیب فیزیکی پین‌ها",
            solutionSteps = "۱. بررسی و تمیزکاری پورت با سوزن پلاستیکی و هوای فشرده\n۲. تست با چند کابل USB-C مختلف (برخی کابل‌های ارزان فقط شارژ را پشتیبانی می‌کنند نه انتقال داده)\n۳. تعویض پورت در صورت تایید آسیب فیزیکی",
            difficulty = "متوسط"
        ),
        TroubleshootingGuide(
            category = TroubleshootingCategory.SOFTWARE,
            brand = "Samsung", model = "همه مدل‌های Galaxy",
            title = "گوشی سامسونگ بعد از فلش، فاقد IMEI یا Baseband Unknown نشان می‌دهد",
            symptoms = "در تنظیمات، IMEI خالی یا نامعتبر است و آنتن‌دهی وجود ندارد",
            possibleCauses = "عدم بک‌آپ‌گیری از پارتیشن EFS پیش از فلش قبلی",
            solutionSteps = "۱. همیشه پیش از فلش هر دستگاه سامسونگ، پارتیشن EFS را بک‌آپ بگیرید (پیشگیری برای آینده)\n۲. برای این دستگاه، بررسی امکان بازیابی EFS از بک‌آپ قبلی (در صورت وجود در باکس تعمیراتی)\n۳. در صورت نبود بک‌آپ، نیاز به نوشتن مجدد IMEI با ابزار تخصصی (با رعایت قوانین و مالکیت دستگاه)",
            difficulty = "تخصصی"
        )
    )
}
